...
import { sampleWithRequiredData } from '../contract-type.test-samples';
...
describe('ContractType Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.contractTypes?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to contractTypeService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getContractTypeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getContractTypeIdentifier).toHaveBeenCalledWith(entity);
